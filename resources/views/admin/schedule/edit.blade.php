    ...
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-600">Perbarui informasi jadwal penerbangan.</p>
            </div>
                ...
                <form method="POST" action="{{ route('admin.schedules.update', $schedule->id) }}" class="space-y-4">
                    ...
                    @method('PUT')
                        ...
                        <div>
                            ...
                            <input name="plane_name" type="text"
                                value="{{ old('plane_name', $schedule->plane_name) }}"
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div>
                                ...
                                <input name="origin" type="text" value="{{ old('origin', $schedule->origin) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900/20" />
                                @error('origin')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                            <div class="sm:col-span-2">
                                ...
                                <input name="departure_time" type="datetime-local"
                                    value="{{ old('departure_time', optional($schedule->departure_time)->format('Y-m-d\\TH:i')) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900/20" />
                            ...
                            <div>
                                ...
                                <input name="stock" type="number" min="0" value="{{ old('stock', $schedule->stock) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900/20" />
                                @error('stock')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div>
                            ...
                            <input name="price" type="number" min="0" value="{{ old('price', $schedule->price) }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900/20" />
                            @error('price')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
